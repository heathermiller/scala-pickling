...
trait PicklerMacros extends Macro {
  def impl[T: c.WeakTypeTag](pickleFormat: c.Tree): c.Tree = {
    ...
    val pickler = {
      c.topLevelRef(syntheticPicklerQualifiedName(tpe, pickleFormat.tpe)) orElse c.introduceTopLevel(syntheticPackageName, {
        ...
        q"""
          class ${syntheticPicklerName(tpe, pickleFormat.tpe)} extends scala.pickling.Pickler[$tpe] {
            import scala.pickling._
            import scala.pickling.`package`.PickleOps
            type PickleFormatType = ${pickleFormat.tpe}
            implicit val pickleFormat = new PickleFormatType()
...
trait UnpicklerMacros extends Macro {
  def impl[T: c.WeakTypeTag]: c.Tree = {
    ...
    val sym = tpe.typeSymbol.asClass
    val unpickler = {
      c.topLevelRef(syntheticUnpicklerQualifiedName(tpe)) orElse c.introduceTopLevel(syntheticPackageName, {
        ...
        q"""
          class ${syntheticUnpicklerName(tpe)} extends scala.pickling.Unpickler[$tpe] {
            import scala.pickling._
